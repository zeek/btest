...
``btest`` accepts the following options:
    ...
    -c CONFIG, --config=CONFIG
        ...
        if found in the current directory. That default can be
        overridden with ``BTEST_CFG`` environment variable.
    ...
    -T, --timing-baseline
        Record new timing baselines for the current host. See
        `timing`_ for more.
...
``TimingDeltaPerc``
    A percent value defining the `timing`_ deviation that's tolerated
    for a test before it's considered failed. Default is 1.0%.
...
``@TEST-MEASURE-TIME``
   ...
   previously established baseline. If it deviates significantly, the
   test will be considered failed. See `timing`_ for more about timing
   test execution.
...
test basis by adding a ``@TEST-RECORD-TIMING`` directive. The test
will then execute as usual yet also record the duration for which it
executes. From then on, further runs on the same host will compare
their times against that baseline and declare a test failed if it
deviates by more than, by default, 1%. (To tune the behaviour, look at
the ``Timing*`` `options`_.)
...
instructions that a tests executes, which tends to be more stable.
That however requires the right tools to be in place. On Linux, BTest
leverages `perf <https://perf.wiki.kernel.org>`_. If a test requests
measurement but BTest can't find it, it will warn and ignore the
request. By default, BTest will search ``perf`` in the ``PATH``; you
can specify a different path to the binary by setting ``PerfPath`` in
``btest.cfg``.
